...
import itertools
from .layer5 import WeightedSumANFISLayer5
...
class Antecedents(torch.nn.Module):
    def __init__(self, n_sets, and_operator=torch.prod):
        ...
        self.n_sets = n_sets  # Lista com o nº de conjuntos por variável
        # Calcula o produto total de regras Num(FP)^Num(Atributos)
        self.n_rules = torch.prod(torch.tensor(n_sets)).item()
        self.and_operator = and_operator
        self.mean_rule_activation = []

        # MESHGRID cria todas as combinações possíveis de índices das regras.
        # Se cada variável tem N conjuntos (0, 1, ..., N), gera todas combinações: (0,0,0,0) até (N,N,N,N)
        grids = torch.meshgrid([torch.arange(s) for s in n_sets], indexing="ij")
        # Transforma os grids em uma matriz de índices de regras (shape: [Num(regras), Num(Atirbutos)])
        rule_indices_tensor = torch.stack([g.reshape(-1) for g in grids], dim=1)
        # register_buffer salva os índices junto com o modelo
    ...
    def forward(self, memberships):
        ...
        # Caso variáveis tenham números diferentes de conjuntos, iguala
        ...
        # Empilha os tensores: shape final [Num(Atributos), Batch_Size, Num(FP)]
        memb_tensor = torch.stack(padded, dim=0)  

        # Mapeamento de regras
        # idx prepara os índices para buscar os valores de pertinência corretos para cada regra
        idx = self.rule_indices.T.unsqueeze(1).expand(-1, N, -1)  
        # gather busca no memb_tensor os valores que compõem cada uma das regras para todas as amostras
        gathered = torch.gather(memb_tensor, 2, idx)

        # Antecedente das regras
        # gathered tem shape [Num(Atributos), Batch, Num(regras)]. Permutamos para [batch, Num(regras), Num(Atributos)]
        # and_operator multiplica os 4 valores de pertinência de cada regra
        # Cada uma das regras vira um único valor de ativação
        antecedents = self.and_operator(gathered.permute(1, 2, 0), dim=2)
        
        # Ajuste caso o operador retorne tuplas
        if isinstance(antecedents, tuple): 
            antecedents = antecedents[0]

        # Retorna a força de cada regra para cada amostra
        return antecedents
...
class Consequents(nn.Module):
    def __init__(self, n_sets, n_classes):
        ...
        if self.mode == 'classification':
            self.linear = nn.Linear(
                in_features=self.n_vars, # Recebe os atributos dos dados
                out_features=self.n_rules * self.n_classes, # Gera Num(regras) * Num(classes) saídas
            )
            
    def forward(self, x):
        # x [Batch_Size, Num(atributos)]
        # consequents [Batch_Size, Num(regras) * Num(classes) saídas]
        # Cada amostra tem Num(regras) * Num(classes) saídas de resultado, Num(classes) sugestões para cada uma das regras
...
class Inference(nn.Module):
    def __init__(self, n_classes, output_activation=nn.Identity(), layer_5_aggregator=WeightedSumANFISLayer5):
        ...
        self.layer_5_aggregator = layer_5_aggregator

    def forward(self, antecedents, consequents):
        # --- Camada 3 ---
        # antecedentes w (batch_size, Num(Regra)): Pesos normalizados
        w = antecedents / (torch.sum(antecedents, dim=1, keepdim=True) + 1e-12)
        if self.mode == 'classification':
            n_rules = w.shape[1]
            # w: muda de [batch_size, Num(Regra)] para [batch_size, Num(Regra), 1] (multiplicar pelas 3 classes)
            w = w.unsqueeze(-1)
            # consequents: muda de [batch_size, Num(Regra)*Num(classes)] para [batch_size, Num(Regra), Num(classes)]
            # para cada uma das regras, um valores de saída por classe.
            consequents = consequents.view(-1, n_rules, self.n_classes)

        # --- Camada 5 ---
        # (w * consequents) Multiplica a força de ativação da regra regra pelo valor de consequente calculado anteriormente
        # torch.sum(..., dim=1) Soma as contribuições de todas as regras
        # O resultado final consolidado em y_hat, [batch_size, Num(classes)], multiplica o peso normalizado (w) pelo valor do consequente da função linear (f)
        #y_hat = torch.sum(w * consequents, dim=1, keepdim=True).squeeze()
        y_hat = self.layer_5_aggregator(w, consequents)
