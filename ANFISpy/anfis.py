...
from ANFISpy.aggregators import *
...
class ANFIS(nn.Module):
    def __init__(
        ...
        self.inference = Inference(self.output_n_classes, output_activation, layer_5_aggregator)
    def forward(self, x):
        # --- CAMADA 1 ---
        # x entra como um lote (ex: batch_size amostras)
        # A lista percorre os atributos de cada amostra
        # Cada mf aplica as FP (Baixo, Médio, Alto) em todos os atributos do dataset
        # Lista com N tensor, onde N é a quantidade de atributos, cada tensor tem forma [Batch_Size, Num(FP)].
        memberships = [mf(x[:, i]) for i, mf in enumerate(self.memberships)]

        # --- CAMADA 2 ---
        # Combina os graus de pertinência usando o agregador fuzzy
        # 'antecedents' terá forma [Batch_Size, Num(atributos)*(Num(FP))]
        antecedents = self.antecedents(memberships)

        # --- CAMADA 4 ---
        # Calcula a parte "então"
        # O modelo linear gera um valor para cada uma das 81 regras multiplicado pelo Num(classes)
        # 'consequents' terá forma [Batch_Size, Num(regras)*(Num(classes)]
        consequents = self.consequents(x)

        # --- CAMADA 3 e 5 ---
        # Realiza a soma ponderada entre a força das regras (antecedents) e seus valores (consequents).
        # Faz a normalização para que as regras que dispararam mais tenham mais peso na decisão (--- CAMADA 3 ---)
        # Y terá forma [Batch_Size, Num(classes)]
    ...
    def print_rules(self, precision=10):
    ...
    def get_MF(self):
        params = {}
        for i, mf_block in enumerate(self.memberships):
            # Obtém o nome da classe para identificar o tipo (ex: 'GaussianMF', 'TriangularMF')
            mf_type = mf_block.__class__.__name__.lower()
            
            input_data = {"type": mf_type}
            
            # Lógica de extração baseada no tipo de MF
            if 'gaussian' in mf_type:
                input_data["mu"] = mf_block.mu.detach().cpu().numpy()
                input_data["sigma"] = mf_block.sigma.detach().cpu().numpy()
                
            elif 'triangular' in mf_type:
                input_data["b"] = mf_block.b.detach().cpu().numpy()
                input_data["deltaL"] = mf_block.deltaL.detach().cpu().numpy()
                input_data["deltaR"] = mf_block.deltaR.detach().cpu().numpy()
                
            elif 'bell' in mf_type:
                input_data["c"] = mf_block.c.detach().cpu().numpy()
                input_data["a"] = mf_block.a.detach().cpu().numpy()
                input_data["b_param"] = mf_block.b.detach().cpu().numpy() # Nomeado b_param para não confundir
                
            elif 'sig' in mf_type:
                input_data["c"] = mf_block.c.detach().cpu().numpy()
                input_data["a"] = mf_block.a.detach().cpu().numpy()
                
            params[f"input_{i}"] = input_data
        return params
...
class CANFIS(nn.Module):
    def __init__(
        ...
        self.inference = Inference(self.output_n_classes, layer_5_aggregator=output_activation)
